    ...
    public:
        ...
        int hash(const char* code) {
            ...
            for(unsigned int i = 0; i < strlen(code); i++) {
                // Assigns letters of the alphabet to the value of 0 -25
                // Each location of the bit, or letter, corresponds to
                // a base of 26 to the power of the location of the bit
                /* For example:
                 *   A    A    B -> 001 = 1 -> 1
                 *   A    B    A -> 010 = 1 -> 26
                 *   ^    ^    ^
                 * 26^2 26^1 26^0
                 * 
                 * This algorithm is more likely to prevent collisions 
                 * because the values of the keys are hashed to
                 * different indexes more than the previous algorithm.
                 */
                value += (code[i] - 'A') * pow(26,i);
            }
